...
def TestInfo( test,indent=0 ):
    ret = 'Test: {0}: {3}\n   File: {1}\n   Directory: {2}\n'.format(test.Name,test.TestFile,test.TestDirectory,ResultType.to_color_string(test._Result))
    if test._Reason:
...
def TestRunInfo( tr,indent=0 ):
    ret = "Run: {0}: {1}\n".format(tr.DisplayString,ResultType.to_color_string(tr._Result))
    if tr._Reason:
...
def ProcessInfo( ps,indent=0 ):
    if ps._Result == ResultType.Unknown:
        ret = 'Process: {0}: {1}\n'.format(ps.Name,ResultType.to_color_string(ResultType.Skipped))
        ps._Reason = "Was not started"
    else:
        ret = 'Process: {0}: {1}\n'.format(ps.Name,ResultType.to_color_string(ps._Result))
    if ps._Reason:
...
def CheckerData( item,indent=0 ):
    ...
    ret = '{0} : {1} - {2}\n'.format("{0}".format(item.DescriptionGroup) if item.DescriptionGroup is not None else "Test", item.Description, result,indent=" " * indent)
...
def TestRunData( obj,indent=0 ):
    ...
    if not obj._Reason:
        ...
        if obj._Result == ResultType.Passed:
            pass # don't print anythign extra for passing tests ( until we add some detailed option)
