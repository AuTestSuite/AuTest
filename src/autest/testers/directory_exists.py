...
class DirectoryExists(tester.Tester):
    def __init__(self,exists,test_value=None,kill_on_failure=False, description_group=None, description=None):
        if description is None:    
            if exists:
                description='Checking that Directory "{0}" exists'.format(tester.get_name(test_value))
            else:
                description='Checking that Directory "{0}" does not exists'.format(tester.get_name(test_value))
