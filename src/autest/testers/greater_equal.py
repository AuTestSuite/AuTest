...
class GreaterEqual(tester.Tester):
    def __init__( self,value,test_value=None,kill_on_failure=False, description_group=None, description=None ):
        ...
        super(GreaterEqual,self).__init__(value=value,
    ...
    def test( self,eventinfo, **kw ):
        ...
        self.Description=self.Description.format(tester.get_name(self.TestValue),self.Value,ev=eventinfo)
        # do test
        if val < self.Value:
        ...
        host.WriteVerbose(["testers.GreaterEqual","testers"],"Passed - " if self.Result == tester.ResultType.Passed else "Failed - ",self.Reason)
