...
class FileExists(tester.Tester):
    def __init__( self,exists,test_value=None,kill_on_failure=False, description_group=None, description=None ):
        if description is None:   
            if exists:
                description = 'Checking that file "{0}" exists'.format(tester.get_name(test_value))
            else:
                description = 'Checking that file "{0}" does not exists'.format(tester.get_name(test_value))
    ...
    def test( self, eventinfo, **kw ):
        ...
        host.WriteVerbose(["testers.FileExists","testers"],"{0} - ".format(tester.ResultType.to_color_string(self.Result)),self.Reason)
