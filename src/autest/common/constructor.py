...
def map_kw(cls,func_info,args,kw):
    ...
    total_default=0 if func_info.defaults is None else len(func_info.defaults)
...
def user_map(func_info,argmap,kw):
    ...
    for cnt,a in enumerate(argmap):
        if cnt+2 > len(func_info.args):
            raise RuntimeError("Invalid mapping of arguments, function takes {0} arguments got {1}".format(len(func_info.args),cnt+2))
...
def call_base(**kw):
    ...
    for c,args in arg_map.items():
        if not is_a.OrderedSequence(args):
            raise TypeError("Argument mapping needs to be a list of tuple type")
    ...
    def constructor_mapper(func):
        ...
        def init_wrapper(*lst,**kw):
            ...
            try:
                # map in the __init__ argments we want to pass based on class type
                for k,v in arg_map.items():
                    if k in kw["__arg_map__"]:
                        raise RuntimeError("class {0} is already been defined for mapping, in valid class mapping".format(k))
                    kw["__arg_map__"][k]=v
            except KeyError:
                # we have not set it yet
                kw["__arg_map__"]=arg_map.copy()
            #get the arguments we need to get for the current class __init__ call
