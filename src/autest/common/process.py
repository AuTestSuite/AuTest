...
if os.name == 'nt':
    ...
    def killtree(self):
        ...
        time.sleep(1)
        win32.TerminateJobObject(
            self._job, -1)  # pylint: disable=protected-access

    def waitTimeOut(process, timeout):
        # WaitForSingleObject expects timeout in milliseconds, so we convert it
        win32.WaitForSingleObject(int_to_handle(
            process._handle), int(timeout * 1000))
