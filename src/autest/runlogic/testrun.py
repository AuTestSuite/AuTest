...
class TestRun_RunLogic(RunLogic):
    ...
    def doStart( self,ev ):
        ...
        # get processes that are own by test object and add them to its list of running processes
        self.__test_processes = [x for x in tmp if x._process._ParentRunable != self.__tr]
    ...
    def Start( self,testrun ):
        ...
        self.__tr._RegisterEvent("starting_logic", self.__tr.StartingEvent, 
                        testers.Lambda(self.doStart,
                            ...
                            description_group="Starting TestRun {0}".format(self.__tr.Name)
                            )
                        )
        ...
        try:
            ...
            if self.__tr.Setup._Result != testers.ResultType.Passed and self.__tr.Setup._Result != testers.ResultType.Warning:
                host.WriteVerbosef("test_logic","Setup failed for Test {0}",self.__tr.Name)
                return False
            # starting event
    ...
    def Poll( self ):
        ...
        if len(tr_processes):
            host.WriteVerbosef(['testrun_logic'],"Stoping processes owned by TestRun")
            self.StopItems(tr_processes,15,2)        
        
        # call finished event
        if self.__start_time:
            self.__tr.FinishedEvent(FinishedInfo(time.time() - self.__start_time))
        else:
