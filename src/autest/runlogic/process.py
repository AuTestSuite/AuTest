...
class Process_RunLogic(RunLogic):
    ...
    def doSetup( self,ev):
        host.WriteVerbosef("testrun_logic","Setup Test {0}",self._process.Name)
        try:
    ...
    def doStart( self ):
        if self.isRunning():
            ...
            return 
        host.WriteVerbosef(['process'],'Start process {0}',self._process.Name)
        ...
        env=self._process._ComposeEnv() # get the correct shell env for the process
        ...
        host.WriteDebugf(["process"],"Calling RunStarted event with {0} callbacks mapped to it",len(self._process.StartedEvent))
    ...
    def Cleanup( self ):
        if self.__call_cleanup:
            ...
            if self.__proc is None:
                event_info = eventinfo.ProcessFinishedInfo(0,None,self.__output)
            else:
                event_info = eventinfo.ProcessFinishedInfo(time.time() - self.__start_time,self.__proc.returncode,self.__output)
                self.__proc = None        
            ...
            #call event
            host.WriteDebug(["process"],"Calling FinishedEvent event with {0} callbacks mapped to it".format(len(self._process.FinishedEvent)))
    ...
    def Start(self,process):
        ...
        if self._process.Setup._Result != testers.ResultType.Passed and self._process.Setup._Result != testers.ResultType.Warning:
            host.WriteVerbosef("test_logic","Setup failed for Test {0}",self._process.Name)
            return False
    ...
    def Stop(self):
        host.WriteVerbosef(['process'],'Stopping process {0}',self._process.Name)
        if self.isRunning():
