...
class Test_RunLogic(RunLogic):
    ...
    def __init__( self ):
        ...
        self.__running_processes = [] # current running process if any (started by test)
    ...
    def doStart( self,ev ):
        ...
        if len(tmp) and is_a.String(tmp[0]):
            ...
            host.WriteVerbosef("test_logic","Test {0}: Starting of processes Failed!",self.__test.Name)
    ...
    def Start( self,test ):
        ...
        host.WriteVerbosef("test_logic","Starting Test \"{0}\"",self.__test.Name)
        # Make sandbox directory for the given test
        host.WriteVerbosef("test_logic","Creating sandbox directory for Test {0}",self.__test.Name)
        ...
        if self.canRunTest():
            ...
            if self.__test.Setup._Result != testers.ResultType.Passed and self.__test.Setup._Result != testers.ResultType.Warning:
                host.WriteVerbosef("test_logic","Setup failed for Test {0}",self.__test.Name)
                return False
            ...
            host.WriteMessagef(" {0}",testers.ResultType.to_color_string(self.__test._Result))
    ...
    def Poll( self ):
        ...
        if self.__delay_time:
            ...
            try:
                ...
                self.__test.RunningEvent(RunningInfo(self.__start_time,time.time()))
            ...
            # remove and processes started by a test run and are not running anymore
            stopped_processes=[x for x in self.__tr_running_processes if not x.isRunning()]
            self.__tr_running_processes=[x for x in self.__tr_running_processes if x.isRunning()]
            ...
            host.WriteVerbosef(["test_logic"],"Finished test run: {0}",self.__current_run.TestRun.Name)
            ...
            if stack_len:
                ...
                if skip:
                    ...
                    while(len(self.__task_stack) != 0):
                        ...
                        tmp._Reason= "Test run {0} failed".format(self.__current_run.TestRun.Name)
                        self.__task_stack = self.__task_stack[1:]
                    ret = False
                else:
                    # start next test
        ...
        if ret == False:
            ...
            # Stop processes
            host.WriteVerbosef(['test_logic'],"Stopping processes owned by Test")
            if hard_stop:
                self.StopItems(self.__running_processes+self.__tr_running_processes)        
            else:
                self.StopItems(self.__running_processes+self.__tr_running_processes,15,2)        
            # call finished event
            if self.__start_time:
                self.__test.FinishedEvent(FinishedInfo(time.time() - self.__start_time))
            else:
    ...
    def readTest( self ):
        ...
        host.WriteVerbose(["test_logic","reading"],'reading test "{0}"'.format(self.__test.Name))
        execFile(fileName,locals,locals)
        host.WriteVerbose(["test_logic","reading"],'Done reading test "{0}"'.format(self.__test.Name))